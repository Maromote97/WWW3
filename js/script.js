...
function reset() {
    if(endloop===0){
        alert("If you want to turn off mobile pictrue, press Mobile button again!");
        return;
    }
...
function draw() {
    ...
    ctx.arc(scales*9+leftrightValue, scales*9+updownValue, 12*scales, 0, Math.PI * 2, true); // Outer circle
...
function moving() {
    if ( check2 === 1 ) {
        if (window.confirm('Infinite loop, would you like to continue)')) {
            endloop=0;
            check2=2;
            anim();
